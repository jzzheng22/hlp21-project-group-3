...
let makeNewWire (model: Model) (srcPortId: string) (tgtPortId: string) (width: int): Wire = 
    ...
    let newVertices = routeWire (Symbol.getPortCoords model.Symbol (CommonTypes.PortId srcPortId)) (Symbol.getPortCoords model.Symbol (CommonTypes.PortId tgtPortId))
...
let ruleOutToIn (model: Model) (wire: Wire) : bool = 
    match Symbol.getPortType model.Symbol (CommonTypes.PortId wire.SourcePortId), Symbol.getPortType model.Symbol (CommonTypes.PortId wire.TargetPortId) with 
...
let ruleWidthEquality (model: Model) (wire: Wire) : bool =
    if Symbol.getPortWidth model.Symbol (CommonTypes.PortId wire.SourcePortId) = Symbol.getPortWidth model.Symbol (CommonTypes.PortId wire.TargetPortId) then 
        true
    else 
        false
...
let update (msg : Msg) (model : Model): Model*Cmd<Msg> =
    ...
    | Symbol sMsg -> 
        ...
        let wList = 
            model.WX
            |> List.map (fun w -> 
                let newVertices = routeWire (Symbol.getPortCoords sm (CommonTypes.PortId w.SourcePortId)) (Symbol.getPortCoords sm (CommonTypes.PortId w.TargetPortId))
    ...
    | AddWire (portId1,portId2) ->
        let unverifiedWire =
            match Symbol.getPortType model.Symbol (CommonTypes.PortId portId1), Symbol.getPortType model.Symbol (CommonTypes.PortId portId2) with
            | CommonTypes.Output , CommonTypes.Input -> makeNewWire model portId1 portId2 (Symbol.getPortWidth model.Symbol (CommonTypes.PortId portId1)) // Wire was drawn from Output to Input
            | CommonTypes.Input , CommonTypes.Output -> makeNewWire model portId2 portId1 (Symbol.getPortWidth model.Symbol (CommonTypes.PortId portId2)) // Wire was drawn from Input to Output
            | _ , _ -> makeNewWire model portId1 portId2 (Symbol.getPortWidth model.Symbol (CommonTypes.PortId portId1)) // Invalid port combination, will be caught by verifyWire

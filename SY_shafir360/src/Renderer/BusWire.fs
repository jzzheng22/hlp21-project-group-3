...
let init n () =
    ...
    let outPort = (List.tryFind (fun (p: Symbol.PortLocal) -> p.Port.PortType = CommonTypes.Output) ports).Value
    let inPort = (List.tryFind (fun (p: Symbol.PortLocal) -> p.Port.PortType = CommonTypes.Input && p.Port.HostId <> outPort.Port.HostId) ports).Value
